function start() {
	...
	document.body.innerHTML += "<div class='container-fluid' style='width: 665px; padding-top: 50px;'><div class='panel panel-default'><div class='panel-heading' style='text-align: left;'><div class='form-group'><label for='inputsm'>JUMLAH EMAIL</label><input type='text' class='form-control input-sm' id='bulkmail' placeholder='30'><br> TOTAL : <span id='hitungYuk'>0 Result</span></div><div class='btn-group mr-2' role='group' aria-label='First group'><button type='button' class='btn btn-success btn-sm' id='pcreate' onclick='create()' >Create Email</button><button type='button' class='btn btn-primary btn-sm' onclick='createbulk()'>Create Bulk</button></div><div class='btn-group mr-2' role='group' aria-label='Second group'><button type='button' class='btn btn-danger btn-sm' onclick='reset()'>Reset</button></div></div><div class='panel-body' id='wadaeemail' style='display: none;'>";
}
...
function createbulk() {
  ...
  jumlah = document.getElementById("bulkmail").value;
  i = 0;
  hitungOK = 0;
  startbulk();
}
